...
```
Claude Desktop / Cursor  →  start_mcp.py (entry point + stdin proxy)
  →  mcp_server/server.py (FastMCP, 17 @mcp.tool()s, session cache)
...
**Package manager:** `uv` (see `uv.lock`). Python 3.10+ required; `.python-version` pins local development to Python 3.11.
...
uv run python -m compileall mcp_server/ tidal_api/
...
uv run pytest tests/test_utils.py::TestFetchAllItems -v

# Run a single test by name
uv run pytest tests/test_utils.py::TestFetchAllItems::test_fetches_multiple_pages -v
...
Use `uv run` for Python and pytest commands so the locked project environment is used. `pyproject.toml` currently has `pytest` as the only dev dependency.

No `.github/` workflow or `ci.yml` is present in this checkout. If CI is added, mirror the local checks above plus the Docker build. No `tasks/` directory is present either; do not refer to it as the source of truth unless it is added.
...
- No enforced formatter or linter is configured in `pyproject.toml`. Follow PEP 8 manually: 4-space indentation, ~100 char line length, two blank lines between top-level definitions.
...
| Constants | `UPPER_SNAKE_CASE` | `SESSION_FILE`, `VALID_SEARCH_TYPES`, `_MAX_PAGES` |
...
`_get_session()` returns a cached `BrowserSession` or raises `SessionError` with a user-facing message for missing, corrupt, expired, or invalid sessions. `tidal_login()` and a successful `tidal_check_login()` invalidate the cache so the next tool call reloads the saved OAuth session.
...
- **New MCP tools** go in `mcp_server/server.py` as `@mcp.tool()` decorated functions. Non-auth tools should call `_get_session()` inside `try`, catch `SessionError`, and delegate business logic to `tidal_api/routes/`.
...
- **Session state** lives in `tidal_api/browser_session.BrowserSession` (a `tidalapi.Session` subclass). The session JSON is persisted to `SESSION_FILE` from `mcp_server/utils.py` (controlled by `TIDAL_SESSION_FILE` env var, defaults to `{tempdir}/tidal-session-oauth.json`). `mcp_server/server.py` caches the loaded session in-process after validation.
...
Tests live in `tests/`. No live TIDAL credentials or network access are needed. `tests/conftest.py` mocks `tidalapi` and `mcp` at import time using `unittest.mock`, then imports the route and server modules.

- `tests/test_utils.py` — unit tests for `bound_limit`, `fetch_all_items`, `format_track_data`
- `tests/test_server.py` — unit tests for `_call()`, `_get_session()`, session cache invalidation, and all 17 MCP tools
- `tests/test_routes.py` — route tests for auth, tracks/recommendations, playlists, and search
- `tests/test_browser_session.py` — `BrowserSession.login_oauth_start()` and URL scheme handling
- `tests/test_mcp_utils.py` — `SESSION_FILE` default and `TIDAL_SESSION_FILE` override behavior

The current suite collects 213 tests.
